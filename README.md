...
* Вход в панель по логину и паролю, второй фактор — TOTP; пароль
  восстанавливается по ссылке из письма.
* Сотрудники заводятся приглашением на почту: администратор вводит адрес
  и роль, человек переходит по одноразовой ссылке из письма и задаёт себе
  пароль. Временные пароли никто не пересылает. SMTP настраивается прямо
  в панели — в конфиги лезть не нужно.
...
* Мониторинг сервера прямо в панели: процессор по ядрам, память, сетевые
  интерфейсы, диски, разделы, температура и самые прожорливые процессы —
  графиками, без Grafana и без единой сторонней библиотеки.
