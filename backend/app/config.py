...
class Settings(BaseSettings):
    ...
    # --- Мониторинг сервера --------------------------------------------------
    #: Где искать procfs и sysfs хоста. Внутри контейнера собственные `/proc`
    #: и `/sys` описывают машину лишь частично: время процессора и память в
    #: них хозяйские, а вот сетевые интерфейсы — свои, контейнерные. Поэтому
    #: compose монтирует хозяйские деревья, а сюда приезжают пути к ним.
    #: Значения по умолчанию — для запуска без контейнера.
    host_proc: Path = Path("/proc")
    host_sys: Path = Path("/sys")
    #: Корень файловой системы хоста. По нему считается занятость разделов:
    #: без монтирования виден только диск контейнера, а он всегда почти пуст.
    host_rootfs: Path = Path("/")
    #: Как часто worker снимает показания и как долго они хранятся в Redis.
    #: Пять секунд — компромисс: чаще график начинает дрожать на шуме, реже —
    #: короткий всплеск нагрузки проходит между замерами незамеченным.
