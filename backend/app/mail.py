...
def wrap_html(
    *,
    ...
    Экранируются ВСЕ текстовые куски, без исключений. Раньше экранировались
    только `greeting` и `button_label`, а `lead`, `fine_print` и `footer`
    подставлялись как есть — то есть соседние параметры одного и того же
    вызова вели себя по-разному, и нигде это не было записано. Держалось всё
    на том, что единственный вызов с пользовательскими данными экранировал
    их сам; следующий, кто пропустит через `lead` название камеры или
    комментарий администратора, получил бы HTML-инъекцию в письмо. Письмо
    читают вне контекста CSP, и подделать в нём кнопку «Задать пароль»
    стоит одной строки.

    Разметку в эти поля передавать нельзя — она приедет читателю текстом.
    Так и задумано: у писем сервиса её и не бывает.
    """
    safe_url = html.escape(url, quote=True)
    lead = html.escape(lead)
    fine_print = html.escape(fine_print)
