 ...
 *                   везде; задержка 2–4 секунды.
 *
 * H.265 умеют оба транспорта, но только там, где у зрителя есть аппаратный
 * декодер: Chrome 136+ и Safari. Программного декодера в Chrome нет намеренно,
 * из-за патентов, поэтому «сменить транспорт» такую камеру не спасает —
 * помогает только перекодирование на сервере.
 *
...
(function () {
  ...
  var NO_FRAMES =
    "Поток подключился, но кадры не приходят. Обычно это H.265: его играют " +
    "только машины с аппаратным декодером. Попробуйте другой транспорт, " +
    "откройте в Safari или включите перекодирование в настройках камеры.";
