...
ask_smtp() {
    ...
    echo "  ${DIM}Это же можно настроить потом мышкой — раздел «Почта» в панели.${R}"
    confirm "Настроить отправку почты (SMTP) сейчас?" "нет" || return 0
...
# показывает администратору. Проще всего настроить почту в самой панели
# (раздел «Почта»), а не здесь. Если всё же здесь — заполните и перезапустите:
